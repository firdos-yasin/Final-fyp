...
const GenerateImages = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setContent(data.image); // ✅ IMPORTANT FIX
      } else {
        toast.error(data.message);
      }
  ...
  return (
    <div className='h-full p-6 flex gap-4'>
      ...
      {/* LEFT */}
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg bg-white p-4 rounded-lg border'>

        <h1 className='text-xl font-semibold flex items-center gap-2'>
          <Sparkles className='text-green-500' /> AI Image Generator
        </h1>
        ...
        <div className='flex flex-wrap gap-2 mt-3'>
          {imageStyle.map((item) => (
            <span
              ...
              ${selectedStyle === item ? 'bg-green-100 text-green-700' : ''}`}
            >
              {item}
            </span>
          ))}
        ...
        <button
          disabled={loading}
          className='w-full mt-5 bg-green-500 text-white py-2 rounded'
        >
      ...
      {/* RIGHT */}
      <div className='w-full max-w-lg bg-white p-4 border rounded-lg'>

        <h2 className='flex items-center gap-2 text-lg font-semibold'>
          <Image className='text-green-500' /> Generated Image
        </h2>
