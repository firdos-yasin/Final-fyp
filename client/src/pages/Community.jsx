...
const Community = () => {
  ...
  const imageLikeToggle = async (id) => {
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message)
        await fetchCreations() // error pe fresh data lo
      }
    } catch (error) {
      toast.error(error.message)
  ...
  return !loading ? (
    <div className='flex-1 h-full flex-col gap-4 p-6'>
      <p className='text-xl font-semibold mb-4'>Community Creations</p>
      <div className='bg-white h-full w-full rounded-xl overflow-y-scroll'>
        ...
        {creations.length === 0 ? (
          ...
          <div className='flex flex-wrap'>
            {creations.filter(c => c.type === 'image').map((creation, index) => (
